...
suppressPackageStartupMessages(load_part1_packages_LFL())
```
...
## 1.2 tidyverseによるデータ整形

### プロジェクト内のデータを読み込む

本書で使用するデータは、プロジェクトルートの`simulation_data`ディレクトリに置かれている。プロジェクトルートの探索やファイル検索は`utility.R`へ集約し、各章では分析対象となるファイル名だけを指定する。

```{r}
#| label: read-daily-stock-return

tbl_daily_stock_return <- read_csv_file_LFL("ch03_daily_stock_return.csv") |>
