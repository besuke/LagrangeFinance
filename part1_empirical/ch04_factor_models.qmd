...
show_table_LFL(tbl_lagged_variable_availability)
```
...
show_table_LFL(tbl_market_weight_check)
```
...
show_table_LFL(tbl_formation_breakpoints)
show_table_LFL(tbl_portfolio_membership_counts)
```

## I-4.4 単変量ポートフォリオ・ソート

### I-4.4.1 企業規模10分位ポートフォリオ
...
show_table_LFL(tbl_size_decile_summary)
```
...
show_table_LFL(tbl_beme_decile_summary)
```
...
show_table_LFL(tbl_capm_results)
show_table_LFL(tbl_capm_fit)
```
...
## I-4.6 2×3ポートフォリオとSMB・HML

### I-4.6.1 Size--BE/MEポートフォリオ
...
show_table_LFL(tbl_ff_portfolio_summary)
```
...
show_table_LFL(tbl_factor_summary)
...
show_table_LFL(tbl_ff3_results)
show_table_LFL(tbl_ff3_fit)
```
...
show_table_LFL(tbl_model_alpha_comparison)
show_table_LFL(tbl_model_fit_comparison)
```
...
tbl_factor_output_summary <- tibble(
  output_file = relative_path_LFL(vec_factor_output_path),
