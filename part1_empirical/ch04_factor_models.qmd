...
反復処理には明示的なループを使わず、`purrr::map()`系を利用する。
...
source('C:/AnalyticFin/Projects/LagrangeFinance/R/utility_LFL.R')
load_part1_packages_LFL()
```

本章では、第1章から第3章と同じ共通ユーティリティを利用する。ファイル探索、CSV読込み、表の表示、グラフのテーマ設定を各章で重複して定義しない。
...
tbl_monthly <- read_derived_csv_LFL("ch05_output1.csv") |>
  mutate(
...
tbl_annual <- read_derived_csv_LFL("ch05_output2.csv") |>
  mutate(
    firm_ID = as.integer(firm_ID),
    year = as.integer(year)
  ) |>
...
vec_derived_data_dir <- file.path(
  get_simulation_data_dir_LFL(),
  "derived"
)

if (!dir.exists(vec_derived_data_dir)) {
  dir.create(vec_derived_data_dir, recursive = TRUE)
}

vec_factor_output_path <- file.path(
  vec_derived_data_dir,
  "ch06_output.csv"
)
...
tbl_factor_output_check <- readr::read_csv(
  vec_factor_output_path,
