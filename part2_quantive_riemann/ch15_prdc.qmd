...
PRDC（Power Reverse Dual Currency）ノートは、為替レートと金利の両方に依存する構造債である。

この章では、QuantiveRiemannで作成したPRDC評価コードを、LagrangeFinanceのQuartoコンテンツとして整理する。

## 17.1 PRDCの基本構造

PRDCは一般に、外貨金利、円金利、為替レートの3つのリスク要因に依存する。
...
QuantLibGauss::qlg_date <- function(x) {
  if (inherits(x, "Date")) {
    x <- format(x, "%Y-%m-%d")
  }
...
QuantLibGauss::qlg_iso <- function(x) {
