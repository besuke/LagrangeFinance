...
- `QuantLibGauss::qlg_ir_build_ois_curve_envs()`, `QuantLibGauss::qlg_trade_ois_swap_py()`, `qlg_swap_*` が利用可能
...
parse_tenor <- function(x) {
  ...
  switch(
    unit,
    d = QuantLibGauss::qlg_period_days(n),
    w = QuantLibGauss::qlg_period_weeks(n),
    m = QuantLibGauss::qlg_period_months(n),
    y = QuantLibGauss::qlg_period_years(n),
...
curve_tbl <- function(curve_obj, n = 200, extrapolate = TRUE) {
  ...
  ref_date_r <- as.Date(QuantLibGauss::qlg_iso(curve_obj$referenceDate()))
  ...
  seq.Date(start_r, end_r - 1, by = "day") |>
    ...
    mutate(
      ...
      is_business_day = purrr::map_lgl(
        accrual_date_chr,
        ~ !Calendar_isHoliday(calendar_obj, QuantLibGauss::qlg_date(.x))
      )
    ) |>
...
QuantLibGauss::qlg_set_eval_date(trade_date_tona)
...
QuantLibGauss::qlg_show_tbl(tona_quotes, "TONA market quotes", n = 20)
```

```{r build-tona-curve}
tona_curve_envs <- QuantLibGauss::qlg_ir_build_ois_curve_envs(
  quotes = tona_quotes,
...
QuantLibGauss::qlg_show_tbl(
  tibble(
    reference_date = QuantLibGauss::qlg_iso(tona_curve$referenceDate()),
    discount_2022_08_23 = tona_curve$discount(QuantLibGauss::qlg_date("2022-08-23"))
  ),
...
```{r jpy-short-ois}
jpy_ois_short <- QuantLibGauss::qlg_trade_ois_swap_py(
  curve_env = list(
...
jpy_ois_fixed_cf_tbl <- QuantLibGauss::qlg_swap_fixed_leg_table(
  jpy_ois_short$swap,
  pedagogical_tona_curve
)

jpy_ois_float_cf_tbl <- QuantLibGauss::qlg_swap_floating_leg_table(
  jpy_ois_short$swap,
  pedagogical_tona_curve
)

QuantLibGauss::qlg_show_tbl(jpy_ois_fixed_cf_tbl, "TONA fixed leg cashflows", n = 10)
QuantLibGauss::qlg_show_tbl(jpy_ois_float_cf_tbl, "TONA floating leg cashflows", n = 10)
```

### Fair rate の手計算チェック

```{r jpy-fair-rate-check}
jpy_eff_df <- pedagogical_tona_curve$discount(QuantLibGauss::qlg_date("2022-08-23"))
jpy_mat_df <- pedagogical_tona_curve$discount(QuantLibGauss::qlg_date("2022-08-30"))
jpy_annuity <- (7 / 365) * pedagogical_tona_curve$discount(QuantLibGauss::qlg_date("2022-09-01"))
...
```{r jpy-daily-forward}
jpy_ois_daily <- QuantLibGauss::qlg_trade_ois_swap_py(
  curve_env = list(
    curve = pedagogical_tona_curve,
    curve_handle = pedagogical_tona_handle,
    index = pedagogical_tona_index,
...
QuantLibGauss::qlg_set_eval_date("2022-08-19")

jpy_daily_float_cf_tbl <- QuantLibGauss::qlg_ois_daily_forward_table(
...
QuantLibGauss::qlg_show_tbl(jpy_daily_float_cf_tbl, "TONA daily forward leg", n = 20)
...
jpy_float_before_tbl <- QuantLibGauss::qlg_swap_floating_leg_table(
  jpy_ois_short$swap,
  pedagogical_tona_curve
)
...
QuantLibGauss::qlg_set_eval_date("2022-08-25")

jpy_float_after_tbl <- QuantLibGauss::qlg_swap_floating_leg_table(
  jpy_ois_short$swap,
  pedagogical_tona_curve
)
...
jpy_fixing_diag_tbl <- QuantLibGauss::qlg_ir_fixing_table(
  swap = jpy_ois_daily$swap,
  curve = pedagogical_tona_curve,
  index = jpy_ois_short$index
)

QuantLibGauss::qlg_show_tbl(jpy_fixing_diag_tbl, "TONA daily fixing decomposition", n = 20)
```
...
QuantLibGauss::qlg_set_eval_date(trade_date_sofr)
...
sofr_curve_envs <- QuantLibGauss::qlg_ir_build_ois_curve_envs(
  quotes = sofr_quotes,
...
QuantLibGauss::qlg_show_tbl(curve_tbl(sofr_curve), "SOFR curve table", n = 12)
```

```{r sofr-ois-swap}
sofr_ois_trade <- QuantLibGauss::qlg_trade_ois_swap_py(
...
sofr_float_cf_tbl <- QuantLibGauss::qlg_swap_floating_leg_table(
  sofr_ois_trade$swap,
  sofr_curve_env$curve
)

QuantLibGauss::qlg_show_tbl(sofr_float_cf_tbl, "SOFR floating leg cashflows", n = 20)

if (nrow(sofr_float_cf_tbl) >= 2) {
  eff_df <- sofr_curve_env$curve$discount(QuantLibGauss::qlg_date(sofr_float_cf_tbl$accrual_end[1]))
  mat_df <- sofr_curve_env$curve$discount(QuantLibGauss::qlg_date(sofr_float_cf_tbl$accrual_end[2]))
...
make_term_sofr_basis_curve <- function(sofr_curve_env,
  ...
  QuantLibGauss::qlg_set_eval_date("2023-09-26")

  term_curve_handle <- RelinkableYieldTermStructureHandle()

  term_index <- IborIndex(
    "TermSofr",
    QuantLibGauss::qlg_period_months(3),
    2,
    USDCurrency(),
    Sofr()$fixingCalendar(),
    "ModifiedFollowing",
...
```{r two-curve-setup}
QuantLibGauss::qlg_set_eval_date("2021-08-02")
...
ix_1y_single <- Tibor(QuantLibGauss::qlg_period_years(1), s_curve_handle)

s_helpers <- list(
  DepositRateHelper(rt_1y, ix_1y_single),
  SwapRateHelper(
    rt_2y,
    QuantLibGauss::qlg_period_years(2),
    jp_calendar,
    freq_annual,
    "ModifiedFollowing",
    dc_a365,
...
QuantLibGauss::qlg_show_tbl(curve_tbl(s_curve), "Single/forecast curve", n = 10)
```
...
single_swap_bundle <- make_vanilla_swap_safe(
  ...
  fixed_tenor = QuantLibGauss::qlg_period_years(1),
  float_tenor = QuantLibGauss::qlg_period_years(1),
...
ix_1y_discount <- Tibor(QuantLibGauss::qlg_period_years(1), d_curve_handle)
...
QuantLibGauss::qlg_show_tbl(curve_tbl(d_curve), "Discount curve", n = 10)
```
...
ix_1y_forward <- Tibor(QuantLibGauss::qlg_period_years(1), f_curve_handle)
...
f_helpers <- list(
  ...
  SwapRateHelper(
    ...
    QuantLibGauss::qlg_period_days(0),
    d_curve_handle
  )
)
...
QuantLibGauss::qlg_show_tbl(curve_tbl(f_curve), "Forward curve", n = 10)
```

### Two-curve forward rate

```{r two-curve-forward-rate}
fwd_dt_1 <- QuantLibGauss::qlg_date("2022-08-04")
fwd_dt_2 <- QuantLibGauss::qlg_date("2023-08-04")
