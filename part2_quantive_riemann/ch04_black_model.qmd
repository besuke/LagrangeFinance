---
title: "第II-4章 株価・為替と幾何ブラウン運動モデル"
execute:
  eval: true
  echo: true
  warning: false
...
### II-4.2.2 Black過程によるヨーロピアン・オプション評価
...
## II-4.3 ブラック–ショールズ–マートンとヨーロピアン・オプション
...
analytic_put_npv <- european_comparison_tbl |>
  dplyr::filter(method == "analytic") |>
  dplyr::pull(npv)

european_comparison_tbl <- european_comparison_tbl |>
  dplyr::mutate(
    difference_from_analytic = npv - analytic_put_npv
  )
...
mc_comparison_tbl <- tibble::tribble(
  ...
  dplyr::mutate(
    difference_from_analytic = npv - analytic_npv
  )
...
crr_500_american_npv <- american_comparison_tbl |>
  dplyr::filter(method == "crr_500") |>
  dplyr::pull(npv)

american_comparison_tbl <- american_comparison_tbl |>
  dplyr::mutate(
    difference_from_crr_500 = npv - crr_500_american_npv
  )
