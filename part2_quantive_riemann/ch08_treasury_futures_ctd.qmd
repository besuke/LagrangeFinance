...
QuantLibGauss::qlg_show_tbl(
  QuantLibGauss::qlg_schedule_dates(bond_bundle$schedule),
...
QuantLibGauss::qlg_show_tbl(
  tibble::tibble(
    settle_date = QuantLibGauss::qlg_iso(settle_date),
...
bond_cf_tbl <- QuantLibGauss::qlg_bond_cashflow_table(bond_obj) |>
  dplyr::mutate(
    pay_date_r = as.Date(pay_date),
    settle_date_r = as.Date(QuantLibGauss::qlg_iso(settle_date)),
    discount_factor = dplyr::if_else(
      pay_date_r >= settle_date_r,
      purrr::map_dbl(
        pay_date,
        ~ tryCatch(
          i_rate_obj$discountFactor(settle_date, QuantLibGauss::qlg_date(.x)),
...
tmp_cf <- QuantLibGauss::qlg_bond_cashflow_table(bond_obj)
...
mar31 <- QuantLibGauss::qlg_date("2023-03-31")
apr21 <- QuantLibGauss::qlg_date("2023-04-21")
sep30 <- QuantLibGauss::qlg_date("2023-09-30")
...
cat(
  "Futures price:", QuantLibGauss::qlg_fmt_num(futures_price, 6),
  ", spot settlement date:", QuantLibGauss::qlg_iso(settle_date),
  "\n"
)
...
repo_rate <- 5.10 / 100
repo_end_date <- QuantLibGauss::qlg_date("2023-07-06")
...
cat(
  ...
  ", repo end date:", QuantLibGauss::qlg_iso(repo_end_date),
  ", repo days:", repo_days,
  ", repo year fraction:", QuantLibGauss::qlg_fmt_num(repo_year_frac, 4),
  "\n"
)
