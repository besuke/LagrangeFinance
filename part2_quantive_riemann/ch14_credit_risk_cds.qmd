...
mid_date_between <- function(dt1, dt2) {
  d1 <- as.Date(QuantLibGauss::qlg_iso(dt1))
  d2 <- as.Date(QuantLibGauss::qlg_iso(dt2))
  mid_r <- d1 + floor(as.numeric(d2 - d1) / 2)
  QuantLibGauss::qlg_date(format(mid_r, "%Y-%m-%d"))
}
...
make_cds_cashflow_tbl_from_schedule <- function(
  ...
  for (i in 2:n_rows) {
    ...
    acc_start_r <- as.Date(QuantLibGauss::qlg_iso(accrual_start[[i]]))
    acc_end_r <- as.Date(QuantLibGauss::qlg_iso(accrual_end[[i]]))
  ...
  trade_date_r <- as.Date(QuantLibGauss::qlg_iso(trade_date))
  ...
  for (i in seq_len(n_rows)) {
    ...
    if (is.null(dt_i)) {
      ...
      dt_i_r <- as.Date(QuantLibGauss::qlg_iso(dt_i))
...
trade_date <- QuantLibGauss::qlg_date("2022-09-19")
QuantLibGauss::qlg_set_eval_date(trade_date)

maturity_date <- QuantLibGauss::qlg_date("2023-09-20")
notional <- 10000000
rf_rate <- 0.10
cds_quote <- 1.30 / 100
recovery_rate <- 0.40
...
trade_date_2 <- QuantLibGauss::qlg_date("2022-08-19")
QuantLibGauss::qlg_set_eval_date(trade_date_2)

cds_tenor_2 <- QuantLibGauss::qlg_period_years(5)
...
jpn_holidays <- c(
  QuantLibGauss::qlg_date("2024-03-20"), QuantLibGauss::qlg_date("2025-03-20"), QuantLibGauss::qlg_date("2026-03-20"),
  QuantLibGauss::qlg_date("2027-03-22"), QuantLibGauss::qlg_date("2026-09-21"), QuantLibGauss::qlg_date("2026-09-22"),
  QuantLibGauss::qlg_date("2026-09-23")
)
...
tona_bundle <- QuantLibGauss::qlg_ir_build_ois_curve_envs(
  quotes = tona_quotes,
...
QuantLibGauss::qlg_set_eval_date(pricing_date_2)
...
curve_check_tbl_2 <- tibble(
  ...
  text = c(QuantLibGauss::qlg_iso(maturity_date_2), NA_character_)
)
