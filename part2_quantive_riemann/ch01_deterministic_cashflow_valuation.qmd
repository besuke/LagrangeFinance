...
市場で直接観測される金利は、必ずしも各満期に対応するスポットレートではない。市場では、OISレート、スワップレート、国債の価格や複利利回りなどが観測される。これらの市場情報と、日数計算規則、利払頻度および受渡日などのマーケットコンベンションを用いて、各満期のゼロクーポン・スポットレートとディスカウントファクター（以下、DF）を順次求める。この手続きをブートストラップという。
...
以下では、日本円OIS市場の基準金利であるTONAR（Tokyo Overnight Average Rate）を用いて構築した金利カーブを例に、スポットレート、DFおよびフォワードレートの関係を確認する。本章ではカーブ構築手法そのものではなく、債券評価の基礎となる金利期間構造の見方を確認する。市場データから金利カーブを構築する具体的な方法については、第II-2章で詳しく扱う。
...
ここでは、TONARカーブを構築するための市場クォートやブートストラップの詳細には立ち入らない。重要なのは、同じ金利期間構造をスポットレート、DF、フォワードレートという異なる観点から表現できることである。これらの関係を理解することが、以降の債券評価や金利デリバティブ評価の基礎となる。

さらに本章では、固定利付債を例として、クリーン価格、ダーティ価格、経過利子、最終利回り、デュレーションおよびコンベクシティを計算する。あわせて、日数計算規則、休日カレンダー、営業日調整、受渡日および利払スケジュールといったマーケットコンベンションを確認する。
...
### II-1.3.4 デュレーションとコンベクシティによる価格変化の近似
...
manual_cf_tbl <- tibble::tibble(
  ...
  dplyr::mutate(
    cashflow = ifelse(
      payment_date == maturity_date_chr,
      cashflow + face_amount,
      cashflow
    ),
