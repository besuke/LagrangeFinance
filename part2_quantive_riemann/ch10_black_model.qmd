...
.lfg_path_value_at <- function(path_obj, i_zero_based) {
...
.lfg_path_tbl <- function(path_obj, path_id = 1L) {
  ...
  tibble::tibble(
    ...
    price = purrr::map_dbl(seq_len(n_point) - 1L, ~ .lfg_path_value_at(path_obj, .x))
  )
}
...
trade_date <- QuantLibGauss::qlg_date("2023-07-21")
maturity_date <- QuantLibGauss::qlg_date("2023-08-26")
...
mc_terminal_tbl <- purrr::map_dfr(
  ...
  function(i) {
    ...
    tibble::tibble(
      path_id = i,
      terminal_price = .lfg_path_value_at(one_path, n_point - 1L)
    )
  }
) |>
  dplyr::mutate(
...
mc_path_tbl <- purrr::map_dfr(
  ...
  function(i) {
    ...
    .lfg_path_tbl(one_path, path_id = i)
  }
)
...
one_path_tbl <- .lfg_path_tbl(one_path_small, path_id = 1L)
...
trade_date_bsm <- QuantLibGauss::qlg_date("2024-03-01")
QuantLibGauss::qlg_set_eval_date(trade_date_bsm)
...
trade_date_sobol <- QuantLibGauss::qlg_date("2024-03-01")
QuantLibGauss::qlg_set_eval_date(trade_date_sobol)
