...
case_eonia_repricing_tbl <- case_eonia_validation$quote_repricing |>
  dplyr::mutate(
    quote_error_bp = 10000 * .data$quote_error
  )
  ...
  dplyr::mutate(
    quotes = purrr::map(
      ...
      function(shift_bp) {
        case_eonia_quotes |>
          dplyr::mutate(
            rate = .data$rate + shift_bp / 10000,
            rate_pct = 100 * .data$rate
          )
      }
    )
  )
...
case_ois_scenario_result_tbl <- case_ois_scenario_result_tbl |>
  ...
  dplyr::select(
    .data$scenario,
    .data$shift_bp,
    .data$contract_fixed_rate,
    .data$fair_rate,
    .data$fair_rate_change_bp,
    payer_npv = .data$npv,
    .data$payer_valuation_change,
    .data$receiver_npv,
    .data$receiver_valuation_change
  )
