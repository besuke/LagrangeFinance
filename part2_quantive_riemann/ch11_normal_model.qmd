...
lfg_curve_table <- function(curve_obj, n = 12, extrapolate = TRUE) {
  if (extrapolate) {
    tryCatch(
  ...
  ref_date <- tryCatch(
    as.Date(QuantLibGauss::qlg_iso(curve_obj$referenceDate())),
...
make_schedule_tbl <- function(schedule_obj) {
  tibble(
    schedule_date = map_chr(
      ...
      ~ QuantLibGauss::qlg_iso(QuantLibGauss::qlg_schedule_date_at(schedule_obj, .x))
    )
  )
}
...
schedule_date_vec <- function(schedule_obj) {
  map(
    seq_len(schedule_obj$size()),
    ~ QuantLibGauss::qlg_schedule_date_at(schedule_obj, .x)
  )
}
...
trade_date <- QuantLibGauss::qlg_date("2023-09-26")
maturity_date <- QuantLibGauss::qlg_date("2023-12-15")
...
sofr_bundle_list <- QuantLibGauss::qlg_ir_build_ois_curve_envs(
  quotes = sofr_quotes,
...
sofr_curve_tbl <- lfg_curve_table(sofr_bundle$curve, n = 12)
...
QuantLibGauss::qlg_schedule_date_at_local <- function(schedule_obj, i_one_based) {
...
make_schedule_tbl <- function(schedule_obj) {
  tibble::tibble(
    schedule_date = purrr::map_chr(
      seq_len(schedule_obj$size()),
      ~ QuantLibGauss::qlg_iso(QuantLibGauss::qlg_schedule_date_at_local(schedule_obj, .x))
    )
  )
}
...
schedule_date_vec <- function(schedule_obj) {
  purrr::map(
    seq_len(schedule_obj$size()),
    ~ QuantLibGauss::qlg_schedule_date_at_local(schedule_obj, .x)
  )
}
# ------------------------------------------------------------
...
QuantLibGauss::qlg_set_eval_date(QuantLibGauss::qlg_date("2023-09-26"))
...
term_sofr_basis_env <- QuantLibGauss::qlg_ir_make_basis_curve(
...
term_sofr_basis_tbl <- QuantLibGauss::qlg_ir_basis_table(
  term_sofr_basis_env,
  ...
  function(i) {
    RateHelperVector_append(
      ...
      OvernightIborBasisSwapRateHelper(
        ...
        QuantLibGauss::qlg_period(term_sofr_basis_data$tenor[i]),
        2L,
        UnitedStates("Settlement"),
        "ModifiedFollowing",
        FALSE,
...
term_sofr_curve_tbl <- lfg_curve_table(term_sofr_curve, n = 10)
...
cap_effective_date <- QuantLibGauss::qlg_date("2023-09-28")
cap_maturity_date <- QuantLibGauss::qlg_date("2024-09-28")
cap_strike <- 0.05
notional <- 10000000
cap_vol <- 0.88 / 100
...
cap_coupon_tbl <- tibble(
  coupon = map(
    ...
    ~ as_floating_rate_coupon(QuantLibGauss::qlg_leg_cashflow_at(cap_leg, .x))
  )
) |>
  mutate(
    ...
    fixing_date = map_chr(coupon, ~ QuantLibGauss::qlg_iso(FloatingRateCoupon_fixingDate(.x))),
    accrual_start = map_chr(coupon, ~ QuantLibGauss::qlg_iso(Coupon_accrualStartDate(.x))),
    accrual_end = map_chr(coupon, ~ QuantLibGauss::qlg_iso(Coupon_accrualEndDate(.x))),
    pay_date = map_chr(coupon, ~ QuantLibGauss::qlg_iso(CashFlow_date(.x))),
    days = map_dbl(
      coupon,
...
swap_effective_date <- QuantLibGauss::qlg_date("2024-09-30")
swap_maturity_date <- QuantLibGauss::qlg_date("2026-09-30")

fixed_schedule <- Schedule(
  swap_effective_date,
  swap_maturity_date,
  QuantLibGauss::qlg_period_years(1),
  UnitedStates("Settlement"),
  "ModifiedFollowing",
  "ModifiedFollowing",
  "Backward",
...
lfg_show_tbl(
  tibble(
    schedule_date = map_chr(fixed_dates, QuantLibGauss::qlg_iso),
...
exercise_date <- QuantLibGauss::qlg_date("2024-09-26")
...
trade_date_plus_1 <- QuantLibGauss::qlg_date("2023-09-27")
